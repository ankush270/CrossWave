...
import { FaComments, FaSpinner, FaExclamationCircle } from 'react-icons/fa';
...
const SellerRFQList = () => {
        ...
        const fetchData = async () => {
            ...
            try {
                ...
                const [chatsResponse, statsResponse] = await Promise.all([
                    axios.get(`http://localhost:5003/api/chats/seller/${user.id}`, {
                        headers: {
                            'Authorization': `Bearer ${localStorage.getItem('token')}`
                        }
                    }),
                    axios.get(`http://localhost:5003/api/chats/seller/${user.id}/stats`, {
                        headers: {
                            'Authorization': `Bearer ${localStorage.getItem('token')}`
                        }
                    })
                ...
                console.log("User data in SellerRFQList:", user);
    ...
    const handleRespond = (chatId) => {
        console.log('Responding to RFQ with ID:', chatId);
        setSelectedChat(chatId); // This will open the chat modal
    };
            ...
            <div className="bg-gradient-to-r from-white to-blue-50 rounded-xl shadow-sm border border-gray-100 p-6">
                ...
                {stats && (
                    <div className="grid grid-cols-2 md:grid-cols-4 gap-6">
                        {[
                            { label: 'Active Requests', value: stats.active, color: 'emerald', status: 'active' },
                            ...
                            { label: 'Total Requests', value: stats.total, color: 'gray', status: 'all' }
                        ].map((stat) => (
                            <button
                                key={stat.label}
            ...
            <div className="space-y-4">
                {filteredChats.length === 0 ? (
                    <div className="text-center text-gray-500 py-12 bg-white rounded-xl shadow-sm border border-gray-100">
                        ...
                        <p className="text-lg font-medium text-gray-600">No {activeFilter} quote requests found</p>
                        <p className="text-sm text-gray-500 mt-1">
                            {activeFilter === 'active' 
                                ? 'Waiting for new quote requests from buyers'
                        ...
                        >
                            <div className="p-6">
                                <div className="flex justify-between items-start">
                                    ...
                                    <button
                                        onClick={() => handleRespond(chat._id)}
                                ...
                                {chat.negotiations.length > 0 && (
                                    <div className="mt-4 pt-4 border-t border-gray-100">
                                        <div className="flex items-center justify-between">
                                            ...
                                            <div className="flex items-center gap-2">
                                                <span className={`px-3 py-1 rounded-full text-xs font-medium ${
                                                    chat.negotiations[chat.negotiations.length - 1]?.proposedBy.role === 'seller'
